#include <eosio.lynx/eosio.lynx.hpp>
...
namespace eosio {
    ...
    void eosiolynx::setperm2(name acc, const std::vector<uint8_t>& perms ){
    ...
    void eosiolynx::setperm(name acc, const std::map<std::string,uint8_t>& perms ){
		require_auth( _self );
	...
	void eosiolynx::setuserava(name acc, std::string ava ){
		
		internal_use_do_not_use::require_auth2("welcome.lynx"_n.value, "update"_n.value );
		//require_auth(acc);
	...
	void eosiolynx::setuserdata(name acc, std::string data ){
	...
	void eosiolynx::setusername(name acc, std::string name ){
	...
	void eosiolynx::userverify(name acc, bool  verified ){

		internal_use_do_not_use::require_auth2("welcome.lynx"_n.value, "newacc"_n.value );
		//require_auth( _self );
		//require_auth(acc);
	...
	void eosiolynx::userprimary(name acc, bool  primary ){
	...
	void eosiolynx::reqperm(name acc, std::string permission ){
    ...
    void eosiolynx::remove(name acc){
    ...
    void eosiolynx::migration(name acc, std::string name, std::string avatar, bool verified, uint64_t date, std::string data, bool primary) {
        require_auth( _self );
	...
	void eosiolynx::setdappconf(uint64_t ram, uint64_t cpu, uint64_t net){
	...
	void eosiolynx::dappreg(name account){
		...
		if (addRam > 10){
			...
			auto act = action(
				permission_level{ "welcome.lynx"_n, "newacc"_n },
                "eosio"_n,
                "buyrambytes"_n,
                std::make_tuple( "welcome.lynx"_n,
                        account,
                        addRam )
			);
			act.send();
...
EOSIO_DISPATCH( eosio::eosiolynx, (setperm)(setperm2)(remove)(reqperm)(setusername)(setuserdata)(setuserava)(userverify)(userprimary)(migration)(dappreg)(setdappconf))