...
namespace eosiosystem {
   ...
   void native::newaccount( const name&       creator,
      ...
      if( creator != get_self() && creator != "lynx"_n) {

         check (system_contract::checkPermission(creator, "createacc")==1, "You are not authorised to create accounts");  // LYNX Check Permissions

         uint64_t tmp = newact.value >> 4;
         bool has_dot = false;
         bool has_dot_after_char = false; // LYNX
         bool was_char = false;
         static const char* charmap = ".12345abcdefghijklmnopqrstuvwxyz"; // LYNX
		 
         for( uint32_t i = 0; i < 12; ++i ) {
           has_dot |= !(tmp & 0x1f);

           if (was_char && !(tmp & 0x1f)) { // LYNX
               has_dot_after_char = true; // LYNX
           }

           char c = charmap[tmp & (i == 0 ? 0x0f : 0x1f)];  // LYNX
           if ( c >= '1' && c <= '5') { // LYNX
               //has_num = true; // LYNX
               was_char = true;  // LYNX
           }  // LYNX
           ...
           //print("char:", c);
           //has_num |= !(tmp & 0x1f); // LYNX
           tmp >>= 5;
         }

         // check (!has_num, "Account name may only include letters."); 	 // LYNX
         check (!has_dot_after_char, "The character '.' in account names is not allowed.");	 // LYNX
         check (newact.to_string().size() > 2, "Minimum 3 character length.");  // LYNX
   ...
   void native::setabi( const name& acnt, const std::vector<char>& abi ) {
      ...
      check (system_contract::checkPermission(acnt, "setcontract")==1, "You are not authorised to setabi.");  // LYNX Check Permissions
   ...
   void native::setcode( const name& account, uint8_t vmtype, uint8_t vmversion, const std::vector<char>& code ) {
      check (system_contract::checkPermission(account, "setcontract")==1, "You are not authorised to setcode.");  // LYNX Check Permissions
   }
   ...
   /* ----------------------------------- */
   /* ------------ LYNX CODE ------------- */
   //check (checkPermission(acc, "")==1, "You are not authorised to setabi");
   uint8_t system_contract::checkPermission(name acc, std::string permission){
      const std::map<std::string,uint8_t> accperm = eosio::eosiolynx::get_priv ( system_contract::lynx_account, acc);
